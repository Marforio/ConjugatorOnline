<!-- src/components/auxiliaries/auxiliaries_scenes/AuxiliariesScene02_Results.vue -->
<template>
    ...
    <v-card class="mx-auto" max-width="900">
      <v-card-text>
        ...
        <div class="results-scroll">
          ...
          >
            <div>
              <strong>Prompt {{ r.prompt_number }}:</strong>
              {{ r.question }}
            </div>
            ...
            <div v-if="r.out_of_time" class="text-caption mt-1">
              ⏱ Out of time
            </div>
          </div>
        </div>
        ...
        <div class="d-flex justify-center align-center mt-4 gap-5">
          ...
          <router-link
            :to="{
              path: '/dashboard',
              query: { tab: 'other-games', game: 'Uses Of Auxiliaries' },
            }"
          >
...
const percent = computed(() => {
  ...
  return Math.round(
    (props.results.correct_count / props.results.total_rounds) * 100
  );
});
